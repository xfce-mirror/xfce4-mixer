...
static void
gst_mixer_sndio_set_volume (GstMixer *mixer, GstMixerTrack *track, gint channels, gint *volumes)
{
  int i;
...
gst_mixer_sndio_set_mute (GstMixer *mixer, GstMixerTrack *track, gboolean mute)
{
  GstMixerSndio *sndio = GST_MIXER_SNDIO (mixer);
  gint channels = NUM_CHANNELS(track);
  g_debug("gst_mixer_sndio_set_mute called on track %s with mute=%d, track has switch=%d, nchan=%d", track->label, mute, HAS_SWITCH(track), channels);
  if (IS_OUTPUT(track)) {
    if (HAS_SWITCH(track)) {
      /* call sioctl_setval for the first mute addr */
      ...
      int i;
      gint* volumes = g_new(gint, channels);
      if (mute) {
        for (i = 0; i < channels; i++)
        {
      ...
      gst_mixer_sndio_set_volume(mixer, track, channels, volumes);
      g_free(volumes);
    }
